...
class BundleContextImpl implements FelixBundleContext
{
    ...
    public String getProperty(String name)
    {
        ...
        {
            m_felix.checkPermission( new java.util.PropertyPermission(name, "read"));
        }
        ...
        throws BundleException
    {
        ...
        Bundle result = m_felix.installBundle(location, is);
        
        m_felix.checkPermission( new AdminPermission(result, AdminPermission.LIFECYCLE));
    ...
    public void addBundleListener(BundleListener l)
    {
        ...
        if (l instanceof SynchronousBundleListener)
            m_felix.checkPermission(  new AdminPermission(m_bundle, AdminPermission.LISTENER));
        ...
        String[] clazzes, Object svcObj, Dictionary dict)
    {
        ...
        if (clazzes != null)
        {
            for (int i = 0;i < clazzes.length;i++)
                m_felix.checkPermission( new ServicePermission(clazzes[i], ServicePermission.REGISTER));
        }
    ...
    public Object getService(ServiceReference ref)
    {
        ...
        if (ref == null)
        {
            ...
            for (int i = 0;(i < objectClass.length) && !hasPermission;i++)
            {
                try
                {
                    m_felix.checkPermission(  new ServicePermission(objectClass[i], ServicePermission.GET));
    ...
    {
        //throw new RuntimeException("Cannot be supported by the Google App Engine since framework cannot save state on disk");
        return null; 
    }
