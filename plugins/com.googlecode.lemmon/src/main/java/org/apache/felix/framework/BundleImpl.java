...
// TODO: SECURITY - We need a security check here.
    ...
    public URL getEntry(String name)
    {
            try
            {
                getFramework().checkPermission(new AdminPermission(this, AdminPermission.RESOURCE));
            }
            catch (Exception e)
            {
    ...
    public Enumeration findEntries(String path, String filePattern, boolean recurse)
    {
            try
            {
                getFramework().checkPermission(new AdminPermission(this,
        ...
        if (sm != null)
        {
            ...
            for (int i = 0; i < refs.length; i++)
            {
                ...
                for (int j = 0; j < objectClass.length; j++)
                {
                    try
                    {
                        getFramework().checkPermission(new ServicePermission(
