...
/**
 * Modified for Lemmon.
 * Added methods for performing secured actions on FileObjects (instead of Files). 
 */
/**
 * <p>
...
public class SecureAction
{
    ...
    public URL createURL(String protocol, String host, int port, String path)
    ...
    public URL createURL(URL context, String spec)
        throws MalformedURLException
    {
        if (securityEnabled())
        {
            ...
            {
                Actions actions = (Actions) m_actions.get();
                actions.set(Actions.CREATE_URL_WITH_CONTEXT_ACTION, context, spec);
                return (URL) AccessController.doPrivileged(actions, m_acc);
            }
            catch (PrivilegedActionException ex)
            {
    ...
    public String getAbsolutePath(final FileObject file)
    {
        return (String)runAction( new PrivilegedAction() { 
            public Object run()
            {
                try
                {
    ...
    public boolean fileExists(final FileObject file)
    {
        ...
        if (securityEnabled())
                return ((Boolean) AccessController.doPrivileged(action, m_acc)).booleanValue();
        return ((Boolean)action.run()).booleanValue();
    }
    ...
    public boolean isFileDirectory(final FileObject file)
    {
        PrivilegedAction action= new PrivilegedAction()
        {
            public Object run()
            {
                try
                {
                    return FileType.FOLDER.equals( file.getType()) ? Boolean.TRUE : Boolean.FALSE;
                }
                catch ( FileSystemException e )
                {
                    throw new RuntimeException(e);
    ...
    public boolean mkdirs(final FileObject file)
    {
        return mkdir( file );
    }
    public boolean mkdir(final FileObject file)    
    {
        return ((Boolean)runAction( new PrivilegedAction()
        {
            public Object run()
            {
                try
    ...
    public FileObject[] listDirectory(final FileObject file)    
    {
        return (FileObject[]) runAction(new PrivilegedAction()
        {
            public Object run()
            {
                try
    ...
    protected final Object runAction(PrivilegedAction action)
    {
        return ((System.getSecurityManager() != null) ? 
            AccessController.doPrivileged(action, m_acc) : action.run());    
    }
    
    protected final Object runAction(PrivilegedExceptionAction action) 
    ...
    public boolean renameFile(final FileObject oldFile, final FileObject newFile)
    {
        return ((Boolean)runAction( new PrivilegedAction()
        {
            public Object run()
    ...
    public InputStream getFileInputStream(final FileObject file) 
    throws IOException
    {
        ...
        {
            if (securityEnabled())
                return (InputStream) AccessController.doPrivileged(action, m_acc);
            return (InputStream) action.run();
        }
        catch ( PrivilegedActionException e )
        {
    ...
    public OutputStream getFileOutputStream(final FileObject file) 
    throws IOException
    {
        PrivilegedExceptionAction action= new PrivilegedExceptionAction()
        {
        ...
        {
            if (securityEnabled())
                return (OutputStream) AccessController.doPrivileged(action, m_acc);
            return (OutputStream) action.run();
        }
        catch ( PrivilegedActionException e )
        {
            if (e.getException() instanceof IOException)
    ...
    public boolean deleteFile(final FileObject target)
    {
        PrivilegedAction action= new PrivilegedAction()
        {
            public Object run() 
            {
                try
                {
                    return target.delete() ? Boolean.TRUE : Boolean.FALSE;
                }
                catch ( FileSystemException e )
                {
                    throw new RuntimeException(e);
    ...
    public JarInputStream openJAR(final FileObject file) throws IOException
    {
        try
        {
            return (JarInputStream) runAction( new PrivilegedExceptionAction() {

                public Object run() throws IOException
                {
                    return new JarInputStream(file.getContent().getInputStream());
                } 
            } );
        }
        catch (PrivilegedActionException ex)
    ...
    public JarInputStream openJAR(final FileObject file, boolean verify) throws IOException
    {
        try
        {
            return (JarInputStream) runAction( new PrivilegedExceptionAction() {
...
*/
    ...
    private static class Actions implements PrivilegedExceptionAction
    {
        ...
        public Object run() throws Exception
        {
            ...
            {
                return ((FileObject) arg1).getContent().getOutputStream();
            }
...
//            }
    ...
    public URL createURL( URL url, String spec, URLStreamHandler svc )
    {
        // lemmon
        // TODO Auto-generated method stub
        throw new RuntimeException("SecureAction.createURL( URL , String , URLStreamHandler ) method not supported in Lemmon");
    }
}