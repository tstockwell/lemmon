...
class JarRevision extends BundleRevision
{
    ...
    private FileObject m_bundleFile = null;

    public JarRevision(
        Logger logger, FileObject revisionRootDir, String location, boolean byReference)
        throws Exception
    {
    ...
    public JarRevision(
        Logger logger, FileObject revisionRootDir, String location,
        ...
        throws Exception
    {
        ...
        if (byReference)
        {
            m_bundleFile = revisionRootDir.getFileSystem().resolveFile( location );
        }
        else
        {
            m_bundleFile = getRevisionRootDir().resolveFile( BUNDLE_JAR_FILE, NameScope.CHILD);
        }
    ...
    public synchronized Map getManifestHeader() throws Exception
    {
        ...
        JarHandler jarFile = null;

        try
        {
            // Open bundle JAR file.
            jarFile = new JarHandler(BundleCache.getSecureAction().openJAR(m_bundleFile));
